// Create admin user for reading and writing data from server
db.createUser({
  user: "admin",
  pwd: "password",
  roles: [
    {
      role: "root",
      db: "admin",
    },
  ],
});

// Create testing data for local environment
// User - email: test@gmail.com, username: test, psw: qweqweqwe
let res = [
  db.users.drop(),
  db.users.insertMany([
    {
      _id: ObjectId("123456789123456789123451"),
      chats: [],
      blockedUsers: [],
      born: new Date("2020-01-01T00:00:00.174Z"),
      status: true,
      activeTopic: "123456789123456789123452",
      username: "test",
      salt: "62a633a7a64abd1909c69aa78259cff1258dd335c8c7a453de09e6801cd46b36",
      hash:
        "",
      createdAt: new Date("2020-01-01T00:00:00.174Z"),
      updatedAt: new Date("2020-01-01T00:00:00.174Z"),
    },
  ]),
  db.topics.drop(),
  db.topics.insertMany([
    {
      _id: ObjectId("123456789123456789123452"),
      title: "Topic Test",
      category: "Categoria 1",
      creatorId: "123456789123456789123451",
      active: true,
      createdAt: new Date("2020-01-01T00:00:00.174Z"),
      updatedAt: new Date("2020-01-01T00:00:00.174Z"),
    },
  ]),
];
printjson(res);
